...
class KVCache:
    ...
    def index_select(self, dim, indices):
        ...
        self.data.copy_(tgt, non_blocking=True)
...
def initialize_past_key_values(large_model, small_model, num_return_sequences=1, two_kv=False, batch_size=1):
    """
    ...
    if two_kv:
        ...
        # batch_size = 1
        # Initializing a tensor to store past keys and values for all layers
        small_past_key_values_data = torch.zeros(
            config.num_hidden_layers * 2,
            batch_size * num_return_sequences,
            config.num_key_value_heads,
            4096,
            config.hidden_size // config.num_attention_heads,
            device=small_model.device,
            dtype=small_model.dtype,
        )
        # Initialize tensor to store the current length of the cached data for all layers.
        # [IMPORTANT] It needs to be kept on CPU for quick access and updates.
        small_current_length_data = torch.zeros(
            config.num_hidden_layers * 2, dtype=torch.long, device="cpu"
        )
        # Creating a KVCache for each pair of key and value in all layers
        small_past_key_values = [] * config.num_hidden_layers
        for i in range(config.num_hidden_layers):
            small_past_key_values.append(
                [
                    KVCache(small_past_key_values_data[i * 2 + j], small_current_length_data[i * 2 + j])
                    for j in range(2)
                ]
            )
        
        return past_key_values, past_key_values_data, current_length_data, small_past_key_values, small_past_key_values_data, small_current_length_data
    else:
