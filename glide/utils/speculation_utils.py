...
# medusa_chose: 我们需要的n-gram及其前缀,无重复 存的topk的k
#     medusa_mask: tree mask
#     tree indices: 拿这个索引所有logit(大模型+4个head)，取出n-gram对应的indices（kv）len(choices)+1
#     candidate: 41个logit
#     tree candidate:  用tree indice索引candidate，得到的token id size:len(choices)+1
#     retrieve indices：从后往前只看叶子节点，得到对应n-gram内每个kv对应的在tree candidate里面对应的位置 size: n,max_proposal length
#     cart_candidates： tree_candidate[retrieve_indices] size: n*max_proposal length
tokenizer = AutoTokenizer.from_pretrained("/home/ducunxiao/model/vicuna-7b-v1.5")
...
def indicate_step(retrieve_indices_nest, medusa_len):
    step_number = [0 for _ in range(medusa_len)]
    for path in retrieve_indices_nest:
        for i in range(len(path)):
            step_number[path[i]+1] = i+1
    return step_number

def get_all_prefix(sequence):
    # 生成前缀数组
    prefixs = set()

    for s in sequence:
        for i in range(1, len(s)+1):
            string = " ".join(list(map(str, s[0:i])))
            prefixs.add(string)
...
def generate_medusa_buffers_joint_llama(medusa_choices, large_last_token=-1, device="cuda", num_beams=1):
    # if num_beams > 1:
    medusa_choices = get_all_prefix(medusa_choices)
    # Sort the medusa_choices based on their lengths and then their values
    sorted_medusa_choices = sorted(medusa_choices, key=lambda x: (len(x), x))
    medusa_len = len(sorted_medusa_choices) + 1
    # print("choices")
    # print(sorted_medusa_choices)
    ...
    # 找到我想要的树的按顺序的每层节点（token）在4*topk+1的logit里面的对应的下标
    # 到时候从4*topk+1的tensor里面取
    medusa_tree_indices = torch.zeros(medusa_len, dtype=torch.long)
    medusa_tree_indices[0] = large_last_token
    # start = 0
    # for i in range(len(depth_counts)):
    #     for j in range(depth_counts[i]):
    #         cur_medusa_choice = sorted_medusa_choices[start + j]
    ...
    for i in range(len(sorted_medusa_choices)):
        medusa_tree_indices[i+1] = sorted_medusa_choices[i][-1]
    # print("medusa tree indices")
    # print(medusa_tree_indices)
    p = [[indice] for indice in medusa_tree_indices]
    # print(tokenizer.batch_decode(p, skip_special_tokens=False))
    ...
    retrieve_steps = indicate_step(retrieve_indices_nest, medusa_len)
    retrieve_steps = torch.tensor(retrieve_steps, dtype=torch.long)
    ...
    medusa_buffers = {
        ...
        "tree_indices": medusa_tree_indices, # 就是proposal扁平化之后的token id
        "medusa_position_ids": medusa_position_ids,
        "retrieve_indices": retrieve_indices, # 用于取回proposal
        "retrieve_steps": retrieve_steps # 指定medusa_tree_indices中的token是来自于第几个step的，speculative sample要用
        }
...
def generate_candidates_joint_llama(tree_candidates, retrieve_indices):
...
def generate_medusa_buffers(medusa_choices, device="cuda"):
    ...
    # tree indice
    # 祖宗（大模型）的token，第一层第0个       第二层的第一个（1+1*10+1）
    # [0,                  1,               12,                23,             24]
    # logit[tree_indice]=我真正涉及到的5个token_id
    
    # retrieve indice
    # 到叶子节点的序列的kv的位置（前缀的位置，包括自己，包括祖宗）倒着看（因为后面的才是叶子，如果碰到了之前看过的前缀，说明不是叶子）
    # [[0, 1, 2, 4], [0, 1, 2, 3]]
    # 用这个取上面得到的token_id
...
def initialize_medusa_for_joint_llama(input_ids, model, medusa_attn_mask, large_past_key_values, small_past_key_values, large_model, large_kv):
    # print("initialize medusa")
    score, outputs, logits = model.medusa_forward(
        input_ids=input_ids, 
    ...
    # large_model.model.medusa_mask = medusa_attn_mask
    return score, logits
...
def reset_medusa_mode(
    ...
    try:
        ...
        large_model.model.medusa_mask = None
        large_model.model.medusa_mode = None
...
def generate_candidates(medusa_logits, logits, tree_indices, retrieve_indices):
    ...
    # 用0表示unk_token_id, -1是之前pad_input()+1的结果，它是padding
    # -1用来索引所以在最后加0
    tree_candidates_ext = torch.cat([tree_candidates, torch.zeros((1), dtype=torch.long, device=tree_candidates.device)], dim=0)
...
def tree_decoding_joint_llama(
    ...
    with torch.inference_mode():
        outputs = model(
            input_ids=tree_candidates.repeat(batch_size, 1),
            use_cache=True,
    ...
    return logits[0, retrieve_indices], logits[0]
...
def evaluate_posterior_joint_llama(
    logits, candidates, temperature, posterior_threshold, posterior_alpha, small_scores=None, tree_logits=None, tree_candidates=None, do_speculative_sample=False, retrieve_indices=None, retrieve_steps=None, candidates_to_small_kv_indice=None
):
    # Greedy decoding based on temperature value
    ...
    logits = sample_method(logits)
    small_scores = sample_method(small_scores)
    
    alpha_per_step = torch.zeros_like(tree_candidates, dtype=torch.float, device=candidates.device)
    accept_mask = torch.ones_like(tree_candidates, dtype=torch.long, device=candidates.device)
    
    # choose tree_candidates to avoid sample unknown token.
    for t in range(1, tree_candidates.size(1)):
        # retrieve_steps用于索引第几步
        sampled_ratios = (
            tree_logits[t, tree_candidates[t]] / small_scores[0, retrieve_steps[t]-1, tree_candidates[t]]
        )
        sampled_ratios = torch.min(sampled_ratios,
                                    torch.ones_like(sampled_ratios))
        rs = torch.rand_like(sampled_ratios)

        cur_alpha = min(logits[batch, t, candidates[batch, t+1]],
                        small_scores[0, t, candidates[batch, t+1]])
        ...
        if rs < sampled_ratios:
            alpha_per_step[0, t] = cur_alpha
            
        else:
            alpha_per_step[0, t] = 0
            accept_mask[0, t] = 0
            break
    
    best_accept_length, best_candidate = torch.max(torch.cumprod(accept_mask[retrieve_indices], dim=-1).sum(dim=1))
    best_alpha = alpha_per_step[retrieve_indices[best_candidate]].sum()
    return {
        "best_candidate": best_candidate,
        "accept_length": best_accept_length,
        "alpha": best_alpha
    }


def update_inference_inputs_joint_llama(
    input_ids,
    candidates,
    best_candidate,
    accept_length,
    ...
    input_ids = torch.cat(
        [input_ids, candidates[None, best_candidate, : accept_length + 1].repeat(input_ids.size(0), 1)], dim=-1
    )

    # Update the past key values based on the selected tokens
    # Source tensor that contains relevant past information based on the selected candidate
    tgt = large_past_key_values_data[..., select_indices, :]
    # Destination tensor where the relevant past information will be stored
    dst = large_past_key_values_data[..., prev_input_len : prev_input_len + tgt.shape[-2], :]
    # Copy relevant past information from the source to the destination
    dst.copy_(tgt, non_blocking=True)

    # Update the current length tensor (currently only support batch size is 1)
    large_current_length_data.fill_(prev_input_len + tgt.shape[-2])
    
    # 这里与是specInf做法一致
    small_current_length_data.fill_(prev_input_len)
